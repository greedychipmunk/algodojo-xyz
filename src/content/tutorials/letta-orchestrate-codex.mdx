...
```mermaid
flowchart TD
    User["You"] -->|message| Agent["Letta Agent<br/>(stateful, persistent memory)"]
    Agent -->|"codex exec prompt"| Codex["Codex CLI<br/>(stateless, isolated context)"]
    Codex -->|"final message"| Agent
    Agent -->|"summary + insights"| User

    subgraph Memory ["Letta Agent Memory Blocks"]
