...
```mermaid
flowchart TD
    User["You"] -->|message| Letta["Letta Agent<br/>(stateful, persistent memory)"]
    Letta -->|"codex exec prompt"| Codex["Codex CLI<br/>(stateless, isolated context)"]
    Codex -->|"final message"| Letta
    Letta -->|"summary + insights"| User

    subgraph Letta ["Letta Agent Memory"]
        Persona["persona<br/>(orchestrator identity)"]
        Codebase["codebase_context<br/>(project knowledge)"]
        History["dispatch_history<br/>(past Codex results)"]
    end

    Letta -.->|"reads"| Persona
    Letta -.->|"reads"| Codebase
    Letta -.->|"writes"| History
```
...
async function main() {
  ...
  const ask = () => rl.question("you > ", async (input) => {
    ...
    for await (const message of session.stream()) {
      switch (message.type) {
        case "reasoning":
          process.stdout.write("\x1b[90m" + message.content + "\x1b[0m");
          break;
        case "tool_call":
          if (!announcedTools.has(message.toolCallId)) {
            announcedTools.add(message.toolCallId);
            console.log("\n\x1b[33m[dispatching to " + message.toolName + "]\x1b[0m");
          }
          break;
        case "tool_result":
          console.log(
            "\x1b[32m[result: " + message.content.slice(0, 200) + (message.content.length > 200 ? "..." : "") + "]\x1b[0m"
          );
          break;
        case "assistant":
...
sequenceDiagram
    ...
    L->>C: codex exec "Investigate race condition in src/auth/login.ts..."
    C-->>L: "Found a race in line 47. How should I fix it?"
    L->>U: "Codex found a race condition. Want me to have it fix it?"
    U->>L: "Yes, fix it"
    L->>C: codex exec resume <session-id> "Fix the race condition you found"
    C-->>L: "Fixed by adding a mutex..."
    L->>U: "Codex fixed it by adding a mutex to the login handler"
```
...
```mermaid
flowchart TD
    Msg["User message"] --> Check{"Already in<br/>memory?"}
    Check -->|"yes"| Direct["Answer directly<br/>(no dispatch)"]
    Check -->|"no"| Scope{"Complex<br/>investigation?"}
    Scope -->|"no, quick lookup"| Dispatch1["Single Codex dispatch<br/>read-only sandbox"]
    Scope -->|"yes, multi-file"| Parallel{"Multiple<br/>independent<br/>areas?"}
    Parallel -->|"yes"| Dispatch2["Parallel Codex dispatch<br/>multiple agents"]
    Parallel -->|"no"| Dispatch3["Single Codex dispatch<br/>workspace-write if editing"]
    Dispatch1 --> Synthesize["Letta synthesizes<br/>and stores learnings"]
