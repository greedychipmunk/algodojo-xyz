...
description: "Wire a stateful Letta agent to social-cli so it researches topics, writes full-length articles, and publishes them to your Leaflet.pub blog on a schedule — no manual intervention required."
tier: "free"
category: "ai"
tags: ["letta", "ai-agents", "social-cli", "leaflet", "automation", "blogging", "python", "advanced"]
difficulty: "advanced"
...
- A local driver **researches** each topic on the web, feeds the results back to the agent, and the agent **writes a full article** in Markdown.
- The driver **publishes** the finished article to your **[Leaflet.pub](https://leaflet.pub)** publication via [social-cli](https://github.com/letta-ai/social-cli).
- A **content calendar** (a YAML file) tracks every topic from proposal through publication so nothing falls through the cracks.

If you've worked through [Build Your First Letta Agent](/tutorials/letta-build-your-first-agent) and [Give Your Letta Agent Custom Tools](/tutorials/letta-agent-tools), you have everything you need. The pattern is the same one from [Automated Social Media Manager](/tutorials/automated-social-media-manager) — a stateful agent supplies the judgment, a thin local driver supplies the hands — but here the output is a researched blog post instead of a social reply.
...
So we split responsibilities the same way:
...
- **A local Python driver is the hands.** It does web research (fetching pages from the open internet), sends the research to the agent, collects the agent's drafted article, and runs `social-cli publish` to push it to Leaflet.
- **A content calendar** (a YAML file) sits between them, tracking each topic through the pipeline: `proposed` → `drafted` → `published`.

```mermaid
flowchart TD
    CC["Content Calendar<br/>(calendar.yaml — topics, status, files)"]
    ...
    subgraph PLAN ["plan mode"]
        direction LR
        P1["Driver"] --> P2["Agent proposes topics"]
        P2 --> P3["Calendar updated"]
    end

    subgraph RESEARCH ["research mode"]
        direction LR
        R1["Driver"] --> R2["Web research"]
        R2 --> R3["Agent writes article"]
        R3 --> R4["Calendar updated"]
    end

    subgraph PUBLISH ["publish mode"]
        direction LR
        PU1["Driver"] --> PU2["social-cli"]
        PU2 --> PU3["Leaflet.pub"]
    end
```

Three cron jobs run the three modes on different schedules. The agent never touches the network — it just thinks and writes.
...
├── manager.py            # the recurring driver (plan / research / publish)
...
The content calendar is a YAML file that tracks every topic through the pipeline. The driver reads and writes it; the agent never touches it directly. Create `calendar.yaml`:
...
| `drafted` | The driver has done web research and the agent has written a full article |
| `published` | The article has been published to Leaflet via `social-cli publish` |

## Step 10 — Write the Driver

The driver is the part that runs on a schedule. It has three modes:

- `plan` — Ask the agent to propose new topics based on the blog's subjects and what's already been published.
- `research` — Pick the next `proposed` topic, do web research, feed the results to the agent, and collect the drafted article.
- `publish` — Pick the next `drafted` article and publish it to Leaflet via `social-cli publish`.
...
def plan() -> None:
    ...
    # Show the agent what's already been published or proposed so it doesn't repeat
    existing = [t["title"] for t in cal.get("topics", [])]
    existing_str = "\n".join(f"- {t}" for t in existing) if existing else "(none yet)"

    subjects_str = ", ".join(cal.get("subjects", SUBJECTS))
    prompt = (
    ...
    for name, args in ask(prompt):
        if name == "propose_topics":
            ...
            for i, t in enumerate(topics):
                topic_id = f"{today}-{len(cal['topics']) + i + 1:03d}"
                cal["topics"].append({
                    "id": topic_id,
...
def research() -> None:
    ...
    cal = load_calendar()
    topic = next((t for t in cal["topics"] if t["status"] == "proposed"), None)

    if not topic:
        print("No proposed topics to research. Run 'plan' first.")
        return

    # Step 1: Do web research
    print(f"Researching: {topic['title']}")
    research_text = do_research(topic["title"], topic["subject"])
    topic["research"] = research_text[:500]

    # Step 2: Ask the agent to write the article with the research as context
    prompt = (
    ...
    for name, args in ask(prompt):
        if name == "draft_article":
            ...
            print(f"Drafted: {title} → {draft_path}")
        elif name == "skip_topic":
            topic["status"] = "skipped"
            print(f"Skipped: {topic['title']} — {args.get('reason', '')}")
...
def publish() -> None:
    ...
    cal = load_calendar()
    topic = next((t for t in cal["topics"] if t["status"] == "drafted"), None)

    if not topic:
        print("No drafted articles to publish. Run 'research' first.")
        return
    ...
    output = sh(cmd)
    print(f"social-cli output: {output}")
...
if __name__ == "__main__":
    ...
    {"plan": plan, "research": research, "publish": publish}[mode]()
```
...
- **The calendar is the source of truth.** The driver reads it to find the next topic to research or the next draft to publish. It writes it after every state transition. If something goes wrong, you can inspect `calendar.yaml` to see exactly where the pipeline stopped.
...
```bash
python manager.py plan       # agent proposes 3-5 topics
python manager.py research   # research + draft the first proposed topic
python manager.py publish    # publish the first draft to Leaflet
```

Watch the output. On `plan`, you'll see one line per topic — "Proposed: ...". On `research`, you'll see "Researching: ..." followed by "Drafted: ... → content/posts/...". On `publish`, you'll see the `social-cli` output and "Published: ...".
...
The whole point is autopilot. Three cron jobs run the pipeline on different cadences:

```text
# crontab -e   (adjust paths to your project + venv)

# Propose new topics every Monday at 9am
0 9 * * 1 cd /home/you/blog-manager && .venv/bin/python manager.py plan >> manager.log 2>&1

# Research and draft one article every Tuesday and Thursday at 9am
0 9 * * 2,4 cd /home/you/blog-manager && .venv/bin/python manager.py research >> manager.log 2>&1

# Publish one article every Wednesday and Friday at 8am
0 8 * * 3,5 cd /home/you/blog-manager && .venv/bin/python manager.py publish >> manager.log 2>&1
```

This gives you a two-article-per-week cadence:

- **Monday:** The agent proposes 3-5 new topics.
- **Tuesday:** The driver researches and drafts the first proposed topic.
- **Wednesday:** The draft is published to Leaflet.
- **Thursday:** The driver researches and drafts the next topic.
- **Friday:** That draft is published.
...
**The agent proposes the same topics repeatedly.** The `plan` mode sends the agent a list of already-proposed and already-published titles and instructs it not to repeat them. If you're still seeing duplicates, check that `calendar.yaml` is being written correctly — the driver needs write permission in the project directory.

**Web research returns no results.** DuckDuckGo's HTML endpoint occasionally rate-limits or blocks automated requests. The driver handles this gracefully — if no results come back, it tells the agent to write from its own knowledge. For more robust research, swap in a dedicated search API (like Tavily or SerpAPI) by replacing the `web_search` function.
...
- **Add a human-in-the-loop gate.** Before the driver publishes, send the draft to yourself on [Telegram](/tutorials/letta-telegram-bot) and only publish on your approval — the safest way to run this against a real publication.
- **Cross-post to Bluesky.** When an article is published to Leaflet, use `social-cli post` to share a short summary with a link on Bluesky. The driver's `publish` mode is a natural place to add this.
- **Add images.** The agent can't generate images from the sandbox, but the driver can. After drafting an article, use an image generation API to create a cover image and reference it in the Markdown before publishing.
...
- **Expand to multiple publications.** If you run several blogs, create multiple agents with different editorial directions and point each one at a different Leaflet publication URI. The driver and calendar structure stay the same.

The pattern underneath all of this is the one worth keeping: a stateful agent supplies the judgment and the consistent voice, a thin local driver supplies the hands and the research, and a content calendar keeps the pipeline honest. Swap the platform, keep the brain.
