...
tier: "free"
category: "ai"
tags: ["letta", "ai-agents", "social-cli", "leaflet", "automation", "blogging", "python", "langgraph", "skills", "advanced"]
difficulty: "advanced"
...
- A **Letta Code skill** — a `SKILL.md` file installed from a GitHub repo — defines the workflow: how to propose topics, draft articles, and skip topics, all via structured JSON output that the pipeline can parse.
...
If you've worked through [Build Your First Letta Agent](/tutorials/letta-build-your-first-agent) and [Give Your Letta Agent Custom Tools](/tutorials/letta-agent-tools), you have everything you need. The pattern is the same one from [Automated Social Media Manager](/tutorials/automated-social-media-manager) — a stateful agent supplies the judgment, a thin local driver supplies the hands — but here the output is a researched blog post instead of a social reply, [LangGraph](https://langchain-ai.github.io/langgraph/) orchestrates the pipeline as a typed state machine, and a [skill](https://docs.letta.com/letta-code/skills/) replaces custom tools with portable, reusable instructions.

## The Architecture

One constraint drives the design, and it's the same one you met in the social media tutorial: the Letta agent runs on a **Letta server**, not on your laptop. That server doesn't have the `social-cli` binary, your Bluesky credentials, or an outbound internet connection for web research.

So we split responsibilities three ways:

- **The Letta agent is the brain.** It holds the editorial direction in memory blocks and makes creative decisions — which topics to propose, how to structure an article, what voice to write in. A **skill** (a `SKILL.md` file installed via Letta Code) is loaded into a memory block so the agent knows the workflow and output format. No custom tools are needed — the agent follows the skill's instructions to respond with structured JSON blocks.
- **A LangGraph pipeline is the hands.** It does web research (fetching pages from the open internet), sends the research to the agent, parses the agent's JSON response, collects the drafted article, and runs `social-cli publish` to push it to Leaflet. LangGraph models the three pipeline stages — plan, research, publish — as nodes in a state graph, with conditional edges that decide what to run next based on the calendar's state.
...
```mermaid
flowchart TD
    START(["Start: load calendar.yaml"]) --> ROUTER["Router<br/>(selects mode)"]
    ...
    PLAN --> RESEARCH
    RESEARCH --> PUBLISH
    PUBLISH --> DONE(["End: save calendar.yaml"])
...
├── create_agent.py       # one-time setup: load skill, create the agent
...
```bash
mkdir -p blog-manager/{state,content/posts}
cd blog-manager
```
...
## Step 7 — Install the Blog Manager Skill

Instead of writing custom Python tools and registering them with the Letta server, you'll install a **skill** — a `SKILL.md` file that tells the agent how to propose topics, draft articles, and skip topics via structured JSON output. Skills are portable across Letta Code and other agent platforms that implement the [Agent Skills](https://agentskills.io) standard.

The skill lives in the [agent-skills repo](https://github.com/greedychipmunk/agent-skills). Install it by asking your Letta Code agent:

```
> Can you install the following skill? https://github.com/greedychipmunk/agent-skills/tree/main/blog-manager
```

Or install it manually by cloning the repo and copying the skill to your global skills directory:

```bash
git clone https://github.com/greedychipmunk/agent-skills.git ~/agent-skills
mkdir -p ~/.letta/skills
cp -r ~/agent-skills/blog-manager ~/.letta/skills/
```

Verify the skill is installed:

```bash
cat ~/.letta/skills/blog-manager/SKILL.md
```

The skill defines three operations — `propose_topics`, `draft_article`, and `skip_topic` — each with a JSON output format wrapped in ```json fences. The agent reads the skill from a memory block (which we'll set up in the next step) and follows its instructions to produce structured responses the pipeline can parse.

## Step 8 — Create the Agent

Now the one-time setup. This script reads the skill's `SKILL.md` file, loads it into a memory block so the agent knows the workflow and output format, and creates an agent whose **memory blocks** encode its editorial direction, target audience, and writing style. Those blocks persist on the server, so the voice stays consistent across every run.
...
# Read the blog-manager skill content
skill_path = Path(os.environ.get(
    "BLOG_MANAGER_SKILL_PATH",
    os.path.expanduser("~/.letta/skills/blog-manager/SKILL.md"),
))
    ...
    memory_blocks=[
        {
            ...
            "value": (
                ...
                "and respond with structured JSON so the driver can act on my "
                "decisions. I never claim to have published something myself — "
...
The skill's content lives in the `blog_workflow` memory block. The agent reads it before every response, so it always knows the output format — no tool registration needed.
...
The shared trick is reading the agent's **structured JSON output** from the response. The blog-manager skill (loaded into the `blog_workflow` memory block in Step 8) instructs the agent to respond with a JSON block wrapped in ```json fences — for example, `{"action": "propose_topics", "topics": [...]}`. We parse that JSON and act on it locally.
...
def ask(prompt: str) -> list[tuple[str, dict]]:
    """Send one prompt to the agent and return parsed (action, data) tuples.

    The agent responds with a JSON block wrapped in ```json fences (as
    defined by the blog-manager skill). We extract and parse each block.
    """
    ...
    # Parse JSON blocks from the text
    calls = []
...
def research_node(state: BlogState) -> dict:
    ...
    prompt = (
        ...
        f"description suitable as a blog excerpt."
    )
    ...
    for action, data in ask(prompt):
        if action == "draft_article":
            ...
            log.append(f"Skipped: {topic['title']} — {data.get('reason', '')}")
...
- **No custom tools are registered.** The agent doesn't call tools to record decisions. Instead, the blog-manager skill (loaded into the `blog_workflow` memory block) instructs the agent to respond with a JSON block wrapped in ```json fences. The `ask()` function parses those blocks with a regex and returns `(action, data)` tuples — the same interface the nodes use, just populated differently.
...
With the Letta server up and `.env` filled in, activate your virtual environment and try each mode by hand:

```bash
source .venv/bin/activate
python3 graph.py plan       # agent proposes topics, then researches + publishes the first one
python3 graph.py research   # research + draft the next proposed topic, then publish it
python3 graph.py publish    # publish the next draft to Leaflet
```
...
If the writing style is off, you don't touch the code — you edit the `editorial_direction` memory block (in the Letta UI at [app.letta.com](https://app.letta.com), or by re-running an updated `create_agent.py` for a fresh agent) and the change applies everywhere. If the output format is off, edit the skill in the [agent-skills repo](https://github.com/greedychipmunk/agent-skills) and re-install it.
...
**Articles are too short or too long.** The blog-manager skill specifies 800-1500 words, but LLMs don't count words precisely. If the output is consistently off, adjust the range in the skill's `SKILL.md` file and re-install it — the agent reads the skill from its memory block before every response.

**The agent doesn't output JSON blocks.** Make sure the skill was loaded into the `blog_workflow` memory block during Step 8. Re-run `create_agent.py` and check that the skill file exists at `~/.letta/skills/blog-manager/SKILL.md`. The agent needs the skill's instructions in its context to know the output format.

**`ModuleNotFoundError: No module named 'yaml'`.** The `pyyaml` package is listed in `requirements.txt` but isn't installed in your current environment. Activate your virtual environment (`source .venv/bin/activate`) and re-run `pip install -r requirements.txt`. If you're not using a venv, install it directly with `pip install pyyaml`.
...
- **Version the skill.** Because the skill lives in a GitHub repo, you can version it, submit pull requests, and roll back changes. Treat the skill like any other piece of infrastructure — review it, test it, and pin a version in production.

The pattern underneath all of this is the one worth keeping: a stateful agent supplies the judgment and the consistent voice, a skill supplies the workflow and output format, a LangGraph pipeline supplies the hands and the research, and a content calendar keeps the pipeline honest. Swap the platform, keep the brain.
